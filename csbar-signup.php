<?php
/** Sets up the csbar Environment. */
require( dirname( __FILE__ ) . '/csbar-load.php' );
add_action( 'csbar_head', 'csbar_no_robots' );
...
if ( is_array( get_site_option( 'illegal_names' ) ) && isset( $_GET['new'] ) && in_array( $_GET['new'], get_site_option( 'illegal_names' ) ) ) {
	csbar_redirect( network_home_url() );
	die();
}
/**
 * Prints signup_header via csbar_head
 *
 * @since MU (3.0.0)
 */
...
add_action( 'csbar_head', 'do_signup_header' );
if ( ! is_multisite() ) {
	csbar_redirect( csbar_registration_url() );
	die();
}
if ( ! is_main_site() ) {
	csbar_redirect( network_site_url( 'csbar-signup.php' ) );
	die();
}
// Fix for page title
$csbar_query->is_404 = false;
/**
 ...
 * @since MU (3.0.0)
 */
function csbarmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'csbar_head', 'csbarmu_signup_stylesheet' );
get_header( 'csbar-signup' );
/**
...
<div class="mu_register csbar-signup-container">
<?php
/**
 ...
 * @param string|csbar_Error $errors     A csbar_Error object containing existing errors. Defaults to empty string.
 */
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	if ( ! empty( $languages ) ) :
		...
		<p>
			...
			<?php
			// Network default.
			$lang = get_site_option( 'csbarLANG' );
			if ( isset( $_POST['csbarLANG'] ) ) {
				$lang = $_POST['csbarLANG'];
			}
			// Use US English if the default isn't available.
			if ( ! in_array( $lang, $languages ) ) {
				$lang = '';
			}
			csbar_dropdown_languages(
				array(
					'name'                        => 'csbarLANG',
					'id'                          => 'site-language',
					'selected'                    => $lang,
	 ...
	 * @since 3.0.0
	 *
	 * @param csbar_Error $errors A csbar_Error object possibly containing 'blogname' or 'blog_title' errors.
	 */
...
function validate_blog_form() {
	...
	return csbarmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'], $user );
}
/**
...
function show_user_form( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param csbar_Error $errors A csbar_Error object containing 'user_name' or 'user_email' errors.
	 */
...
function validate_user_form() {
	return csbarmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
}
/**
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	$current_user = csbar_get_current_user();
	if ( ! is_csbar_error( $errors ) ) {
		$errors = new csbar_Error();
	}
	 ...
	 *     @type csbar_Error $errors     A csbar_Error object possibly containing 'blogname' or 'blog_title' errors.
	 * }
	 */
	...
	<form id="setupform" method="post" action="csbar-signup.php">
 ...
 * @global csbar_Error        $errors     Existing errors in the global scope.
...
function validate_another_blog_signup() {
	...
	if ( ! empty( $_POST['csbarLANG'] ) ) {
		$languages = signup_get_available_languages();
		if ( in_array( $_POST['csbarLANG'], $languages ) ) {
			$language = csbar_unslash( sanitize_text_field( $_POST['csbarLANG'] ) );
			if ( $language ) {
				$blog_meta_defaults['csbarLANG'] = $language;
			}
		}
	}
	/**
	...
	$blog_id = csbarmu_create_blog( $domain, $path, $blog_title, $current_user->ID, $meta, get_current_network_id() );
	if ( is_csbar_error( $blog_id ) ) {
		return false;
	}
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array(), $blog_id = 0 ) {
	if ( $blog_id ) {
		...
		$home_url  = home_url( '/' );
		$login_url = csbar_login_url();
		restore_current_blog();
	} else {
		$home_url  = 'http://' . $domain . $path;
		$login_url = 'http://' . $domain . $path . 'csbar-login.php';
	}
	$site = sprintf(
		'<a href="%1$s">%2$s</a>',
		esc_url( $home_url ),
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 *     @type csbar_Error $errors     A csbar_Error object with possible errors relevant to the sign-up user.
	 * }
	 */
	...
	<form id="setupform" method="post" action="csbar-signup.php" novalidate="novalidate">
		<input type="hidden" name="stage" value="validate-user-signup" />
		<?php
		/** This action is documented in csbar-signup.php */
...
function validate_user_signup() {
	...
	/** This filter is documented in csbar-signup.php */
	csbarmu_signup_user( $user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function signup_blog( $user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type csbar_Error $errors     A csbar_Error object with possible errors relevant to new site creation variables.
	 * }
	 */
...
function validate_blog_signup() {
	...
	$user_result = csbarmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
	...
	$result     = csbarmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'] );
	$domain     = $result['domain'];
	$path       = $result['path'];
	$blogname   = $result['blogname'];
	$blog_title = $result['blog_title'];
	...
	if ( ! empty( $_POST['csbarLANG'] ) ) {
		...
		if ( in_array( $_POST['csbarLANG'], $languages ) ) {
			$language = csbar_unslash( sanitize_text_field( $_POST['csbarLANG'] ) );
			if ( $language ) {
				$signup_meta['csbarLANG'] = $language;
			}
		}
	}
	/** This filter is documented in csbar-signup.php */
	$meta = apply_filters( 'add_signup_meta', $signup_meta );
	csbarmu_signup_blog( $domain, $path, $blog_title, $user_name, $user_email, $meta );
 ...
 */
$active_signup = apply_filters( 'csbarmu_active_signup', $active_signup );
...
if ( 'none' === $active_signup ) {
	...
	$login_url = csbar_login_url( network_site_url( 'csbar-signup.php' ) );
...
<?php
/**
 * Fires after the sign-up forms, before csbar_footer.
 *
 * @since 3.0.0
 */
